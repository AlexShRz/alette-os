...
export type ProgressEventListenerOrEventListenerObject =
	| ProgressEventListener
	| ProgressEventListenerObject;
...
export class XMLHttpRequestEventTarget {
	...
	addEventListener(
		eventType: string,
		listener?: ProgressEventListenerOrEventListenerObject,
	) {
		eventType = eventType.toLowerCase();
		this.listeners[eventType] = this.listeners[eventType] || [];
		this.listeners[eventType].push(
			(listener as ProgressEventListenerObject).handleEvent ||
				(listener as ProgressEventListener),
		);
	}
	removeEventListener(
		eventType: string,
		...
		const index = this.listeners[eventType].indexOf(
			(listener as ProgressEventListenerObject).handleEvent ||
				(listener as ProgressEventListener),
		);
		if (index < 0) {
