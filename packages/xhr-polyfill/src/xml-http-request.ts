...
export class XmlHttpRequest extends XMLHttpRequestEventTarget {
	...
	private _userAgent =
		`Mozilla/5.0 (${os.type()} ${os.arch()}) node.js/${process.versions.node} v8/${process.versions.v8}`;
	...
	private _sendHxxpRequest() {
		...
		const requestMethod: (options: RequestOptionsHttp) => ClientRequest =
			hxxp.request.bind(hxxp);
