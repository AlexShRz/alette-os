import { setContext } from "../../../application";
import {
	path,
	factory,
	reloadable,
	runOnMount,
	tapCancel,
} from "../../../domain";
import { createTestApi } from "../../utils/createTestApi";

test("it is triggered when a request is cancelled", async () => {
	const { custom } = createTestApi();
	const logged: any[] = [];

	const getData = custom(
		runOnMount(false),
		reloadable(() => true),
		tapCancel(async () => {
			logged.push(1);
		}),
		factory(() => {
			return new Promise(() => {});
		}),
	);

	const { execute, cancel } = getData.mount();
	execute();
	cancel();
	execute();
	cancel();
	await vi.waitFor(() => {
		expect(logged).toEqual([1, 1]);
	});
});

test("it is not triggered if we have no request to cancel", async () => {
	const { custom } = createTestApi();
	const logged: any[] = [];

	const getData = custom(
		runOnMount(false),
		reloadable(() => true),
		tapCancel(async () => {
			logged.push(1);
		}),
		factory(() => {
			return new Promise(() => {});
		}),
	);

	const { cancel } = getData.mount();
	cancel();
	cancel();
	await vi.waitFor(() => {
		expect(logged).toEqual([]);
	});
});

test("it can access request props and context", async () => {
	const { api, custom } = createTestApi();
	const context = { asdasdnasd: "asdas" };
	api.tell(setContext(context));

	let caughtContext: typeof context | null = null;
	let caughtPath: string | null = null;

	const pathValue = "/asdads";

	const getData = custom(
		path(pathValue),
		runOnMount(false),
		reloadable(() => true),
		tapCancel(async ({ context, path }) => {
			caughtContext = context as any;
			caughtPath = path;
		}),
		factory(() => {
			return new Promise(() => {});
		}),
	);

	const { execute, cancel } = getData.mount();
	execute();
	cancel();

	await vi.waitFor(() => {
		expect(caughtContext).toBe(context);
		expect(caughtPath).toBe(pathValue);
	});
});

test("it can be combined", async () => {
	const { custom } = createTestApi();
	const logged: number[] = [];

	const getData = custom(
		runOnMount(false),
		reloadable(() => true),
		tapCancel(async () => {
			logged.push(1);
		}),
		factory(() => {
			return new Promise(() => {});
		}),
		tapCancel(async () => {
			logged.push(2);
		}),
		tapCancel(() => {
			logged.push(3);
		}),
	);

	const { execute, cancel } = getData.mount();

	execute();
	cancel();
	await vi.waitFor(() => {
		expect(logged).toStrictEqual([1, 2, 3]);
	});
});
